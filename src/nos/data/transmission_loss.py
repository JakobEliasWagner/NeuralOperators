...
class TLDatasetCompactWave(OperatorDataset):
    ...
    def __init__(self, path: pathlib.Path, n_samples: int = -1):
        ...
        # introduce waves to represent properties
        vals = torch.einsum("abc,ade->adc", u, x)  # different modes for the properties
        vals = (
            (vals - torch.min(vals)) / (torch.max(vals) - torch.min(vals)) - 0.5
        ) * 2  # rescale to prevent aliasing
        # superposition of waves with properties as frequencies
        u = torch.sum(
            torch.stack([torch.sin(2 * torch.pi * vals[:, :, i]) for i in range(vals.size(2))], dim=2), dim=2
        ).unsqueeze(2)

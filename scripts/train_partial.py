...
def main():
    ...
    dataset_sizes = [int(a) for a in torch.round(torch.linspace(230, 461, 4)).tolist()]
    freq_sizes = [int(a) for a in torch.round(torch.linspace(129, 257, 4)).tolist()]

    confs = product(dataset_sizes, freq_sizes)
    out_dir = pathlib.Path.cwd().joinpath("run", "smaller_data")
    for i, (dataset_size, freq_size) in enumerate(confs):
        run_dir = out_dir.joinpath(f"dset_{dataset_size}_freq_{freq_size}_run_{i}")

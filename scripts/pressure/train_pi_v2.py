...
def run():
    ...
    operator = deserialize(
        pathlib.Path.cwd().joinpath("finished_pressure", "const", "ddo_wo", "DeepDotOperator_2024_05_08_17_57_25")
    )
    ...
    with mlflow.start_run(run_name="PI-DDO"):
        ...
        for x, u, y, v in val_loader:
            ...
            # prepare pde part
            tmp_cp = cp.expand(u.size(0), -1, -1).to(device)
            tmp_scp = dataset.transform["y"](tmp_cp).to(device)
            u_pde = dataset.transform["u"].undo(u).detach().to(device)

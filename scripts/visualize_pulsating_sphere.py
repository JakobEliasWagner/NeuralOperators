...
def plot_three_tile_plot(dataset: OperatorDataset, operator: Operator, out_dir: pathlib.Path):
    ...
    for i, (x, u, y, v) in enumerate(plot_loader):
        ...
        axs[0, 0].plot(y_plot, v_plot[:, 0], "r--", label="$\Re(Y_{gt})(x)$")
        axs[0, 0].plot(y_plot, v_plot[:, 1], "g--", label="$-\Im(Y_{gt})(x)$")
        axs[0, 0].plot(y_plot, v_pred_plot[:, 0], "r-", label="$\Re(Y_{pred})(x)$")
        axs[0, 0].plot(y_plot, v_pred_plot[:, 1], "g-", label="$-\Im(Y_{pred})(x)$")
        ...
        axs[1, 1].plot(v_plot[:, 2], y_plot, "r--", label="$\Re(Y_{gt})(y)$")
        axs[1, 1].plot(v_plot[:, 3], y_plot, "g--", label="$-\Im(Y_{gt})(y)$")
        axs[1, 1].plot(v_pred_plot[:, 2], y_plot, "r-", label="$\Re(Y_{pred})(y)$")
        axs[1, 1].plot(v_pred_plot[:, 3], y_plot, "g-", label="$-\Im(Y_{pred})(y)$")
...
def main():
    ...
    operator_dir = pathlib.Path.cwd().joinpath(
        "run", "2024_03_28_18_38_05-381e718b-50e1-4637-99e2-9965f89e7b69", "DeepONet_2024_03_28_18_57_47"
    )
    operator = deserialize(operator_dir)

    # load dataset
    data_path = pathlib.Path.cwd().joinpath("data", "test", "pulsating_sphere_450")
    scale_path = pathlib.Path.cwd().joinpath("data", "train", "pulsating_sphere_450")
    dataset_scale = PressureBoundaryDataset(data_dir=scale_path, n_samples=-1)
    dataset = PressureBoundaryDataset(data_dir=data_path, n_samples=5)

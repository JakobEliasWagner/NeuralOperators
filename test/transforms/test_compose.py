...
from nos.transforms import Compose
...
def test_compose_forward(random_negative_tensor, plus_one_times_two):
    assert torch.allclose(plus_one_times_two(random_negative_tensor), (random_negative_tensor + 1.0) * 2.0)
...
def test_compose_backward(random_negative_tensor, plus_one_times_two):
    ...
    assert torch.allclose(plus_one_times_two.backward(transformed_tensor), random_negative_tensor)
